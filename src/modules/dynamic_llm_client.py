    ...
    try:
        if hasattr(st, "runtime"):
            ...
            if hasattr(st.runtime, "scriptrunner"):
                get_ctx = getattr(st.runtime.scriptrunner, "get_script_run_ctx", None)
                if get_ctx:
                    try:
...
class DynamicLLMClient:
    ...
    def __init__(self, provider: str = None, model: str = None, api_key: str = None):
        ...
        self.provider = (
            provider
            or (st.session_state.get("selected_provider") if ctx else None)
            or LLM_CONFIG["provider"]
        )
        # Chọn model: ưu tiên tham số, sau đó session_state, cuối cùng LLM_CONFIG
        self.model = (
            model
            or (st.session_state.get("selected_model") if ctx else None)
            or LLM_CONFIG["model"]
        )
        ...
        try:
            if self.provider == "google":
                ...
                try:
                    ...
                    logger.info(
                        f"Google API key validated. Available models: {len(models)}"
                    )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"OpenRouter API key validation returned status: {response.status_code}"
                        )
                except Exception as e:
    ...
    def _gen_openrouter(self, messages: List[str]) -> str:
        ...
        try:
            ...
            if res.status_code == 401:
                ...
                raise ValueError(
                    "OpenRouter API Unauthorized: Vui lòng kiểm tra API key."
                )
